...
#   (see scene_stream.py):
RM_PALETTE: Dict[PenColor, Tuple[int, int, int]] = {
...
def lookup_pen_color(pen_color: PenColor,
                     explicit_color_rgba: Optional[Tuple[int, int, int, int]]) -> Tuple[int, int, int, int]:
    if explicit_color_rgba is not None:
        return explicit_color_rgba
    if pen_color in RM_PALETTE:
        color = RM_PALETTE[pen_color]
    else:
        logger.warning(f"{pen_color.name} ({pen_color.value}) as no associated rgb. Falling back on back.")
...
class Pen:
    ...
    def get_segment_color(self, speed, direction, width, pressure, last_width):
        if isinstance(self.base_color, int):
            print()
    ...
    @classmethod
    def create(cls, pen_nr: PenType, color: Tuple[int, int, int, int], width):
        # Brush
        ...
        # Calligraphy (spelling mistake in rmscene will eventually be fixed)
            ...
            return Calligraphy(width, color)
        # Marker
        ...
        # Mechanical Pencil
            ...
            return MechanicalPencil(width, color)
        # Highlighter
            ...
            width = 12
            return Shader(width, color)
        # Erase area
        elif pen_nr == PenType.ERASER_AREA:
            return EraseArea(width, color)
        # Eraser
        elif pen_nr == PenType.ERASER:
            # color = 2 # TODO not sure it is required
